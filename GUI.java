...
public class GUI extends JFrame implements ActionListener {
    ...
    private boolean isRunning;//判断是否还在运行
    private Thread thread;//线程的状态
    ...
    public void initGUI() {
        ...
        JPanel backPanel, centerPanel, bottomPanel;//用于生成按钮
        JLabel jWidth, jLength, jNowGeneration;//用于显示文本或图像
        backPanel = new JPanel(new BorderLayout());//BorderLayout（边界布局）
        centerPanel = new JPanel(new GridLayout(maxWidth, maxLength));//GridLayout（网格布局）
        bottomPanel = new JPanel();
        this.setContentPane(backPanel);//获得JFrame的内容面板
        backPanel.add(centerPanel, "Center");//将网格布置在中间
        backPanel.add(bottomPanel, "South");//将按钮布置在下方
        ...
        lengthList = new JComboBox();//下拉列表组件
        for (int i = 3; i <= 100; i++)//设置按钮
        ...
        jbNowGeneration = new JButton(""+cell.getNowGeneration());//Button不能直接添加int，故采用此方式
        ...
        bottomPanel.add(jLength);//底部增加按钮
    ...
    public GUI(String name) {
        super(name);		//通过 super 关键字调用父类的有参构造方法
        initGUI();
    }
